...
void ASSERT_CALLBACK(AsyncTestContext *testContext, 
	int scannedCnt, int detectedCnt, int completedCnt, int cancelledCnt, int errorCnt)
{
	if(testContext == nullptr)
		return;

	if(scannedCnt >= 0)
		ASSERT_IF(testContext->scannedCnt, scannedCnt);
	if(detectedCnt >= 0)
		ASSERT_IF(testContext->detectedCnt, detectedCnt);
	if(completedCnt >= 0)
		ASSERT_IF(testContext->completedCnt, completedCnt);
	if(cancelledCnt >= 0)
		ASSERT_IF(testContext->cancelledCnt, cancelledCnt);
	if(errorCnt >= 0)
		ASSERT_IF(testContext->errorCnt, errorCnt);
}
...
BOOST_AUTO_TEST_CASE(scan_files_async_positive)
{
	...
	ASSERT_IF(testCtx.scannedCnt == 4 || testCtx.scannedCnt == 0, true); // in case of delta
...
BOOST_AUTO_TEST_CASE(scan_dir_positive)
{
	...
	ASSERT_CALLBACK(&testCtx, -1, 0, 1, 0, 0); //scanned, detected, completed, cancelled, error
...
BOOST_AUTO_TEST_CASE(scan_dir_root)
{
	...
	ASSERT_CALLBACK(&testCtx, -1, 8, 1, 0, 0); //scanned, detected, completed, cancelled, error
...
BOOST_AUTO_TEST_CASE(scan_dir_media)
{
	...
	ASSERT_CALLBACK(&testCtx, -1, 1, 1, 0, 0); //scanned, detected, completed, cancelled, error
...
BOOST_AUTO_TEST_CASE(scan_cancel_positiive)
{
	...
	ASSERT_CALLBACK(&testCtx, -1, 0, 0, 1, 0); //scanned, detected, completed, cancelled, error
...
BOOST_AUTO_TEST_CASE(delta_scan_basic)
{
	...
	ASSERT_CALLBACK(&testBaseCtx, -1, 3, 1, 0, 0); //scanned, detected, completed, cancelled, error
	BOOST_REQUIRE_MESSAGE(testBaseCtx.scannedCnt != 0, "Base Scan count should not be zero");

	// Rescan the same dir
	AsyncTestContext testRescanCtx;

	ASSERT_IF(csr_cs_scan_dir_async(context, TEST_DIR_APPS, &testRescanCtx), CSR_ERROR_NONE);

	std::unique_lock<std::mutex> lRescan(testRescanCtx.m);
	testRescanCtx.cv.wait(lRescan);
	lRescan.unlock();

	ASSERT_CALLBACK(&testRescanCtx, 0, 3, 1, 0, 0); //scanned, detected, completed, cancelled, error
	...
	ASSERT_CALLBACK(&testRescanSubCtx, 0, 1, 1, 0, 0); //scanned, detected, completed, cancelled, error
...
BOOST_AUTO_TEST_CASE(delta_scan_changed_after_scan)
{
	...
	ASSERT_CALLBACK(&testRescanCtx, 1, 3, 1, 0, 0); //scanned, detected, completed, cancelled, error

	ASSERT_DETECTED_IN_LIST(testRescanCtx.detectedList,
		TEST_WGT_APP_ROOT, MALWARE_HIGH_NAME, MALWARE_HIGH_SEVERITY, MALWARE_HIGH_DETAILED_URL);
	ASSERT_DETECTED_IN_LIST_EXT(testRescanCtx.detectedList,
		TEST_TPK_APP_ROOT, true, TEST_WGT_PKG_ID);
