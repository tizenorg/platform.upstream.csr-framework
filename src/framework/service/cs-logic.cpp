...
Db::Cache CsLogic::scanAppDelta(const std::string &pkgPath, const std::string &pkgId)
{
	...
	Db::Cache cache;
	DEBUG("before : " << pkgPath);
	cache.pkgPath = pkgPath;
	DEBUG("after : " << cache.pkgPath);
	DEBUG("before : " << pkgId);
	cache.pkgId = pkgId;
	DEBUG("after : " << cache.pkgId);
...
RawBuffer CsLogic::scanApp(const CsContext &context, const std::string &pkgPath)
{
	...
	auto riskiest = std::unique_ptr<CsDetected>(new CsDetected(*(cache.riskiest)));
	...
	if (history && after && riskiest) {
		if (*history < *riskiest) {
			...
			this->m_db->insertCache(cache, history->isIgnored, riskiestPath);
		...
		if (*history < *riskiest) {
			...
			this->m_db->insertCache(cache, false, riskiestPath);
		...
		if (!rows.empty()) {
			...
			if (worse) {
				this->m_db->insertCache(cache, false, worse->fileInAppPath);
