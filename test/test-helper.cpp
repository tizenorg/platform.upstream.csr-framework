					 ...
					 csr_cs_severity_level_e e_severity, const char *e_detailed_url)
{
	...
	ASSERT_SUCCESS(csr_cs_detected_get_severity(detected, &a_severity));
	ASSERT_SUCCESS(csr_cs_detected_get_malware_name(detected, &a_malware_name.ptr));
	ASSERT_SUCCESS(csr_cs_detected_get_detailed_url(detected, &a_detailed_url.ptr));
						 ...
						 const char *e_file_name, bool e_is_app, const char *e_pkg_id)
{
	...
	ASSERT_SUCCESS(csr_cs_detected_get_timestamp(detected, &a_timestamp));
	ASSERT_SUCCESS(csr_cs_detected_get_file_name(detected, &a_file_name.ptr));
	ASSERT_SUCCESS(csr_cs_detected_is_app(detected, &a_is_app));
	ASSERT_SUCCESS(csr_cs_detected_get_pkg_id(detected, &a_pkg_id.ptr));
...
void ASSERT_DETECTED_HANDLE(csr_cs_detected_h expected, csr_cs_detected_h actual)
{
	...
	ASSERT_SUCCESS(csr_cs_detected_get_severity(expected, &e_severity));
	ASSERT_SUCCESS(csr_cs_detected_get_malware_name(expected, &e_malware_name.ptr));
	ASSERT_SUCCESS(csr_cs_detected_get_detailed_url(expected, &e_detailed_url.ptr));
	ASSERT_SUCCESS(csr_cs_detected_get_timestamp(expected, &e_timestamp));
	ASSERT_SUCCESS(csr_cs_detected_get_file_name(expected, &e_file_name.ptr));
	ASSERT_SUCCESS(csr_cs_detected_is_app(expected, &e_is_app));
	ASSERT_SUCCESS(csr_cs_detected_get_pkg_id(expected, &e_pkg_id.ptr));

	ASSERT_SUCCESS(csr_cs_detected_get_severity(actual, &a_severity));
	ASSERT_SUCCESS(csr_cs_detected_get_malware_name(actual, &a_malware_name.ptr));
	ASSERT_SUCCESS(csr_cs_detected_get_detailed_url(actual, &a_detailed_url.ptr));
	ASSERT_SUCCESS(csr_cs_detected_get_timestamp(actual, &a_timestamp));
	ASSERT_SUCCESS(csr_cs_detected_get_file_name(actual, &a_file_name.ptr));
	ASSERT_SUCCESS(csr_cs_detected_is_app(actual, &a_is_app));
	ASSERT_SUCCESS(csr_cs_detected_get_pkg_id(actual, &a_pkg_id.ptr));
							 ...
							 csr_cs_severity_level_e severity, const char *detailed_url)
{
	...
	for (auto &d : detectedList) {
		ASSERT_IF(csr_cs_detected_get_file_name(d, &a_file_name.ptr), CSR_ERROR_NONE);
		if (strcmp(file_name, a_file_name.ptr) != 0)
			continue;

		ASSERT_SUCCESS(csr_cs_detected_get_malware_name(d, &a_name.ptr));
		ASSERT_SUCCESS(csr_cs_detected_get_severity(d, &a_severity));
		ASSERT_SUCCESS(csr_cs_detected_get_detailed_url(d, &a_detailed_url.ptr));
								 ...
								 const char *file_name, bool is_app, const char *pkg_id)
{
	...
	for (auto &d : detectedList) {
		ASSERT_SUCCESS(csr_cs_detected_get_file_name(d, &a_file_name.ptr));
		if (strcmp(file_name, a_file_name.ptr) != 0)
			continue;

		ASSERT_SUCCESS(csr_cs_detected_is_app(d, &a_is_app));
		ASSERT_SUCCESS(csr_cs_detected_get_pkg_id(d, &a_pkg_id.ptr));
