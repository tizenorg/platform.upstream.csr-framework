...
Db::Cache CsLogic::scanAppDelta(const FilePtr &pkgPtr)
{
	...
	auto visitor = FsVisitor::create([&](const FilePtr &file) {
		...
		cache.detecteds.emplace_back(file->getPath(), std::move(candidate));

		if (cache.riskiestIndex == -1 ||
			*(cache.detecteds[cache.riskiestIndex].second) < *candidate)
			cache.riskiestIndex = cache.detecteds.size() - 1;
					 ...
					 CsDetectedPtr &malware)
{
	...
	auto &riskiest = cache.riskiestIndex == -1 ? dummy : cache.detecteds[cache.riskiestIndex].second;
	auto riskiestPath = cache.riskiestIndex == -1 ? std::string() : cache.detecteds[cache.riskiestIndex].first;
